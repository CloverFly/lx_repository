...
@interface LXPopoverController(Private)
...
-(void)addObservers
{
    ...
    [[NSNotificationCenter defaultCenter] addObserver:self
                                             selector:@selector(willPresentNewPopover:) name:@"LXNewPopoverPresented" object:nil];
...
-(id)initWithViewController:(UIViewController*)viewController
{
    ...
    if(self)
    {
        self.arrowDirection = LXPopoverArrowDirectionAny;
        self.view.userInteractionEnabled = YES;
        _touchView = [[LXTouchView alloc] initWithFrame:self.view.bounds];
        ...
        _contentView = [[LXPopoverView alloc] initWithFrame:CGRectMake(0, 0, 
...
-(void)setTint:(LXPopoverTint)tint
{
...
- (void)viewDidLoad
{
    ...
    [_contentView setArrowDirection:LXPopoverArrowDirectionUp];
    [_contentView addContentView:_viewController.view];
...
-(void)presentPopoverFromPoint:(CGPoint)fromPoint
{
    ...
    [[NSNotificationCenter defaultCenter] postNotificationName:@"LXNewPopoverPresented" object:self];
}

-(CGPoint)originFromView:(UIView*)fromView
{
    CGPoint p;
    if([_contentView arrowDirection] == LXPopoverArrowDirectionUp)
    {
        p.x = fromView.frame.origin.x + fromView.frame.size.width/2.0;
        p.y = fromView.frame.origin.y + fromView.frame.size.height;
    }
    else if([_contentView arrowDirection] == LXPopoverArrowDirectionDown)
    {
        p.x = fromView.frame.origin.x + fromView.frame.size.width/2.0;
        p.y = fromView.frame.origin.y;        
    }
    else if([_contentView arrowDirection] == LXPopoverArrowDirectionLeft)
    {
        p.x = fromView.frame.origin.x + fromView.frame.size.width;
        p.y = fromView.frame.origin.y + fromView.frame.size.height/2.0;
    }
    else if([_contentView arrowDirection] == LXPopoverArrowDirectionRight)
    {
...
-(CGRect)bestArrowDirectionAndFrameFromView:(UIView*)v
{
    ...
    LXPopoverArrowDirection bestDirection;
    
    //if the user wants vertical arrow, check if the content will fit vertically 
    if(LXPopoverArrowDirectionIsVertical(self.arrowDirection) || 
       (self.arrowDirection == LXPopoverArrowDirectionAny && best_h >= best_w))
    {

        //ok, will be vertical
        if(ht == best_h || self.arrowDirection == LXPopoverArrowDirectionDown)
        {
            //on the top and arrow down
            bestDirection = LXPopoverArrowDirectionDown;
        ...
        else
        {
            ...
            bestDirection = LXPopoverArrowDirectionUp;
    ...
    else 
    {
        ...
        if(wl == best_w || self.arrowDirection == LXPopoverArrowDirectionRight)
        {
            //on the left and arrow right
            bestDirection = LXPopoverArrowDirectionRight;
        ...
        else
        {
            ...
            bestDirection = LXPopoverArrowDirectionLeft;
