...
#define LX_POPOVER_ARROW_HEIGHT 10.0
#define LX_POPOVER_ARROW_BASE 20.0
#define LX_POPOVER_RADIUS 10.0


@interface LXPopoverView(Private)
-(void)setupViews;
@end
...
-(CGPathRef)createContentPathWithBorderWidth:(CGFloat)borderWidth arrowDirection:(LXPopoverArrowDirection)direction
{
    CGFloat w = self.bounds.size.width;
    CGFloat h = self.bounds.size.height;
    CGFloat ah = LX_POPOVER_ARROW_HEIGHT; //is the height of the triangle of the arrow
    CGFloat aw = LX_POPOVER_ARROW_BASE/2.0; //is the 1/2 of the base of the arrow
    CGFloat radius = LX_POPOVER_RADIUS;
    CGFloat b = borderWidth;
    
    CGRect rect;
    if(direction == LXPopoverArrowDirectionUp)
    {
    ...
    else if(direction == LXPopoverArrowDirectionDown)
    {
        rect.size.width = w - 2*b;
        rect.size.height = h - ah - 2*b;
        rect.origin.x = b;
    ...
    else if(direction == LXPopoverArrowDirectionRight)
    {
    ...
    else if(direction == LXPopoverArrowDirectionLeft)
    {
        rect.size.width = w - ah - 2*b;
        rect.size.height = h - 2*b;
        rect.origin.x = ah + b;
    ...
    //right arrow
    if(direction == LXPopoverArrowDirectionRight)
    {
    ...
    //down arrow
    if(direction == LXPopoverArrowDirectionDown)
    {
    ...
    //left arrow
    if(direction == LXPopoverArrowDirectionLeft)
    {
...
-(CGGradientRef)createGradient
{
    ...
    if(self.tint == LXPopoverBlackTint)
    {
        if(_arrowDirection == LXPopoverArrowDirectionUp)
        {
    ...
    else if(self.tint == LXPopoverLightGrayTint)
    {
        if(_arrowDirection == LXPopoverArrowDirectionUp)
        {
    ...
    else if(self.tint == LXPopoverRedTint)
    {
        if(_arrowDirection == LXPopoverArrowDirectionUp)
        {
    ...
    else if(self.tint == LXPopoverGreenTint)
    {
        if(_arrowDirection == LXPopoverArrowDirectionUp)
        {
...
-(void)setupViews
{
    ...
    else if(_arrowDirection == LXPopoverArrowDirectionDown)
    {
    ...
    else if(_arrowDirection == LXPopoverArrowDirectionRight)
    {
        contentRect.origin = CGPointMake(10, 40);        
    ...
    else if(_arrowDirection == LXPopoverArrowDirectionLeft)
    {
        contentRect.origin = CGPointMake(10 + LX_POPOVER_ARROW_HEIGHT, 40);        
        contentRect.size = CGSizeMake(self.bounds.size.width-40, self.bounds.size.height-50);
        _titleLabel.frame = CGRectMake(10, 10, self.bounds.size.width-20, 20);           
    }
